...
import { getAcademicBlocks, getAcademicDocuments, getSessionProfile, getSyllabi, getTeacherAccessStatuses, type AcademicBlock, type AcademicDocument, type SessionProfile, type SyllabusDocument, type TeacherAccessStatus } from "@/lib/academic";
...
function TeacherView({ active, done, complete, blocks, profile }: { active: string; done: number[]; complete: (id: number) => void; blocks: AcademicBlock[]; profile: SessionProfile }) {
  ...
  if (active === "Documentos") return <Documents admin={false} blocks={blocks} />;
...
function Documents({admin,blocks=[]}:{admin:boolean;blocks?:AcademicBlock[]}) {
  const [selected,setSelected]=useState("Sílabos");
  const [syllabi,setSyllabi]=useState<SyllabusDocument[]>([]); const [academicDocuments,setAcademicDocuments]=useState<AcademicDocument[]>([]); const [loading,setLoading]=useState(true); const [error,setError]=useState("");
  const assignedCourseKey=[...new Set(blocks.map(block=>block.courseId))].sort().join(",");
  useEffect(()=>{let current=true;const assignedCourseIds=assignedCourseKey?assignedCourseKey.split(","):[];Promise.all([getSyllabi(admin?undefined:assignedCourseIds),getAcademicDocuments()]).then(([syllabusRows,academicRows])=>{if(current){setSyllabi(syllabusRows);setAcademicDocuments(academicRows)}}).catch(reason=>{if(current)setError(reason instanceof Error?reason.message:"No se pudieron cargar los documentos.")}).finally(()=>{if(current)setLoading(false)});return()=>{current=false}},[admin,assignedCourseKey]);
  const counts:Record<string,number|undefined>={"Sílabos":syllabi.length,"Documentos académicos":academicDocuments.length};
  return <>
    <PageTitle eyebrow="RECURSOS" title={admin?"Gestión de documentos":"Documentos"} copy={admin?"Publica y organiza los recursos institucionales para docentes.":"Encuentra sílabos, formatos y documentos académicos en un solo lugar."} action={admin?<button className="primary" title="Carga administrativa preparada para una siguiente etapa">＋ Agregar documento</button>:undefined}/>
    <div className="category-grid">
      {docCats.map((category,index)=>{
        const count=counts[category];
        const available=count!==undefined;
        const content=<><i aria-hidden="true">{["▤","▱","↻","▥","▣","•••"][index]}</i><div><h2>{category}</h2><p>{available?(loading?"Cargando…":`${count} ${count===1?"archivo":"archivos"}`):"Próximamente"}</p></div>{available&&<span className="category-action">Ver documentos →</span>}</>;
        return available
          ? <button type="button" className={`category-card category-card-button ${selected===category?"selected":""}`} key={category} onClick={()=>setSelected(category)} aria-pressed={selected===category}>{content}</button>
          : <article className="category-card unavailable" key={category}>{content}</article>;
      })}
    </div>
    {selected==="Sílabos"?<section className="panel syllabus-panel"><PanelHead title="Sílabos — 2026-II" link={admin?`${syllabi.length} disponibles`:"Según tus cursos asignados"}/>{loading&&<div className="empty-state">Cargando sílabos autorizados…</div>}{error&&<DocumentLoadError error={error}/>} {!loading&&!error&&syllabi.length===0&&<div className="empty-state">No tienes sílabos disponibles para tus cursos actuales.</div>}<div className="syllabus-grid">{syllabi.map(document=><SyllabusCard key={document.id} document={document}/>)}</div></section>:selected==="Documentos académicos"?<section className="panel syllabus-panel"><PanelHead title="Documentos académicos" link={`${academicDocuments.length} disponibles`}/>{loading&&<div className="empty-state">Cargando documentos académicos…</div>}{error&&<DocumentLoadError error={error}/>}<div className="syllabus-grid">{academicDocuments.map(document=><AcademicDocumentCard key={document.id} document={document}/>)}</div></section>:null}
  </>
}

function DocumentLoadError({error}:{error:string}){return <div className="demo-note login-error"><b>!</b><span><strong>No se pudieron cargar los documentos</strong>{error}</span></div>}
...
function AcademicDocumentCard({document}:{document:AcademicDocument}) {
  const size=document.sizeBytes?`${(document.sizeBytes/1024/1024).toFixed(2)} MB`:"PDF";
  return <article className="syllabus-card"><div className="pdf-icon"><b>PDF</b><span>▤</span></div><div className="syllabus-copy"><span className="course-code">{document.documentCode} · V{document.documentVersion}</span><h2>{document.title}</h2><small>{size} · Documento académico</small></div><div className="syllabus-actions"><a href={document.viewUrl} target="_blank" rel="noreferrer" className="primary">Ver documento</a><a href={document.downloadUrl} className="download-link">Descargar</a></div></article>
}

function Tutorials({admin}:{admin:boolean}) { const tutorials=portalTutorials.filter(tutorial=>tutorial.active); return <><PageTitle eyebrow="APRENDIZAJE" title={admin?"Gestión de tutoriales":"Tutoriales"} copy={admin?"Consulta los videos disponibles y prepara la incorporación de nuevos recursos.":"Videos institucionales para ayudarte con los principales procedimientos docentes."} action={admin?<button className="primary" title="Administración de nuevos tutoriales preparada para una siguiente etapa">＋ Registrar video</button>:undefined}/><div className="filter-row"><button className="active">Todos</button>{[...new Set(tutorials.map(tutorial=>tutorial.category))].map(category=><button key={category}>{category}</button>)}</div><div className="tutorial-grid">{tutorials.map((tutorial,index)=><article className="tutorial" key={tutorial.id}><div className={`video-cover c${index}`}><a href={tutorial.videoUrl} target="_blank" rel="noopener noreferrer" aria-label={`Abrir ${tutorial.title} en una nueva pestaña`}>▶</a><span>VIDEO</span></div><div><span className="badge">VIDEO TUTORIAL</span><h2>{tutorial.title}</h2><p>{tutorial.description}</p><small>Disponible en OneDrive institucional</small><a className="tutorial-link" href={tutorial.videoUrl} target="_blank" rel="noopener noreferrer">Ver tutorial →</a></div></article>)}</div>{admin&&<section className="panel tutorial-admin-note"><b>Gestión de tutoriales</b><p>La estructura está preparada para incorporar, clasificar y administrar nuevos videos posteriormente.</p></section>}</> }
