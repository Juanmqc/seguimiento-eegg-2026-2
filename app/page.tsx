...
function Documents({admin,blocks=[]}:{admin:boolean;blocks?:AcademicBlock[]}) {
  ...
  useEffect(()=>{let current=true;const assignedCourseIds=assignedCourseKey?assignedCourseKey.split(","):[];getSyllabi(admin?undefined:assignedCourseIds).then(data=>{if(current)setSyllabi(data)}).catch(reason=>{if(current)setError(reason instanceof Error?reason.message:"No se pudieron cargar los sílabos.")}).finally(()=>{if(current)setLoading(false)});return()=>{current=false}},[admin,assignedCourseKey]);
